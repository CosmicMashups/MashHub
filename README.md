...
For auth email redirects, also add:
...
- **Auth redirects**: Set `VITE_SITE_URL` to your deployed domain in production (for GitHub Pages: `https://<owner>.github.io`) so signup/magic-link confirmation URLs target the correct host. The app appends `${BASE_URL}auth/callback` automatically.
