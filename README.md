...
**💡 Seeing "Supabase is unavailable"?** See [SUPABASE_QUICK_FIX.md](SUPABASE_QUICK_FIX.md) for instant solutions, or run `npm run diagnose`.

For auth email redirects in **local dev**, also add:

- `VITE_SITE_URL=http://localhost:5173`
- `NEXT_PUBLIC_SITE_URL=http://localhost:5173` (optional compatibility alias)

For **production** (e.g. GitHub Pages), set `VITE_SITE_URL` to your public origin including `base` path, e.g. `https://<owner>.github.io/MashHub`. If it stays `localhost` in a production build, confirmation emails can point at localhost until you fix secrets or rely on the app’s runtime correction (see [docs/DEPLOY.md](docs/DEPLOY.md)).
...
- **Auth redirects**: Set `VITE_SITE_URL` (and optionally `NEXT_PUBLIC_SITE_URL`) to your deployed **origin including the Vite `base` path** (for this repo: `https://<owner>.github.io/MashHub`, not only `https://<owner>.github.io`). See [`getAuthRedirectUrl()` in `src/lib/siteUrl.ts`](src/lib/siteUrl.ts) for how the callback URL is built from `VITE_SITE_URL` and `import.meta.env.BASE_URL`. In the **Supabase Dashboard → Authentication → URL Configuration**, set **Site URL** to that same public root and add **Redirect URLs** including `https://<owner>.github.io/MashHub/auth/callback` plus any local dev callbacks you use. If env is still localhost while users open the deployed site, `getSiteUrl()` prefers `window.location.origin` so emails target the site they actually use.
...
| **[SUPABASE_QUICK_FIX.md](SUPABASE_QUICK_FIX.md)** | **🚨 Quick fixes for "Supabase unavailable" message** |
| [SUPABASE_CONNECTION_GUIDE.md](SUPABASE_CONNECTION_GUIDE.md) | Complete troubleshooting guide for connection issues |
| [FIX_RLS_GUIDE.md](FIX_RLS_GUIDE.md) | Row Level Security policy setup |
| [SUPABASE_KEYS_EXPLAINED.md](SUPABASE_KEYS_EXPLAINED.md) | Understanding Supabase API keys |
